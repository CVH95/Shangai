...
int main()
{
    ...
    for(size_t j; j<tsp.size(); j++)
    {
        ...
        _o1 = C.threshold_so2_signal(o1, o_prev);

        modified_output.push_back(_o1);
        o_prev = o1;
    }

    // Storing data to later plots:
    ofstream fs1;
    fs1.open("../genfiles/Modified_CPG_test.csv");
    int st = ntw_out.size();
    for (int i; i<st; i++)
    {
        fs1 << get<0>(ntw_out[i]) << "," << get<1>(ntw_out[i]) << "," << modified_output[i] << endl;
    }
    fs1.close();

    cout << "CPG values stored as <time, output_C1, output_C2>" << endl;
    cout << "Test Finished. Results saved to $ROOT/genfiles/Modified_CPG_test.csv" << endl;
